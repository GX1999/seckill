...
（3）分布式session问题：登录成功后生成唯一标识标记该用户，在redis中存储该标识和用户信息，并向cookie中写入（键值对形式），客户端在随后的访问中都在cookie加上该token。若用户第一次登录，则正常输入登录信息，否则自动填充登录信息。
...
配合redis消亡时间设置每个用户在一定时间内最大可请求次数。使用redis的increment的原子操作，increment是向redis请求Increby命令，返回值是更新后的数据。若不存在对应key时默认初始为0。

（3）使用RabbitMQ削峰

如果用户符合当前秒杀条件，则将秒杀请求发送到队列，然后接收端会从消息队列中取出消息进行异步下单操作。最后前端轮询查询秒杀结果。发送端发送message对象（转化为JSON格式进行传输），接收端使用@RabbitListener声明一个方法监听特定的队列，接收message（将JSON转为Object）进行相应处理。消息队列传输数据格式一般为String类型或对象，传输时都需要序列化。对象序列化会包含类信息等信息，相比于String类型序列化代价更大。一般转为String(JSON)在进行传输。
...
（1）本项目可能出现缓存一致性问题的地方是Redis库存（例如数据库层面通过事务保证发生订单故障后库存正确，但是redis中无法恢复到正确数量），但是Redis在这里起到的是过滤请求的作用，真正在减库存时会在数据库中判断。因此不需要考虑缓存一致性问题。
...
（1）简单模式：无交换机，发送端发送，接收端接收

（2）Fanout发布订阅模式：有交换机，交换机绑定若干队列，将消息转发到所有队列

（3）Direct模式：交换机通过key绑定若干队列，消息转发匹配对应key，然后转发到对应队列

（4）Topic模式：key是模糊匹配

## 9. 如果减库存成功，下订单失败，如何保障库存数量正确？

使用Spring事务@Transactional，将下订单和减库存两个操作放到一个事务中，失败后进行回滚。

## 10. 秒杀项目困难的地方？

如何应对瞬时大流量高并发，从2个方面进行优化
（1）层层限流：在不同的层次尽可能地过滤掉无效请求：用户限流-验证码-Redis中秒杀请求判断（超买、超卖）-消息队列
（2）优化项目，提高并发访问能量：动静分离、Redis缓存、内存标记
