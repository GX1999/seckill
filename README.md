...
（5）用户轮询查询结果：秒杀订单有三种结果（goodsId：查询订单表存在用户ID+商品ID订单，表示秒杀成功、-1：若redis中存在isStockEmpty+商品ID，则代表当前商品库存为0，秒杀失败、0：不属于前两种情况则代表仍在排队中）。用户每间隔1s，查询一次当前秒杀结果。
...
（4）用户轮询查询结果：秒杀订单有三种结果（goodsId：查询订单表存在用户ID+商品ID订单，表示秒杀成功、-1：若redis中存在isStockEmpty+商品ID，则代表当前商品库存为0，秒杀失败、0：不属于前两种情况则代表仍在排队中）。用户每间隔1s，查询一次当前秒杀结果。
